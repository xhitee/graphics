...
int main()
{
    ...
    // Create the power-up item
    sf::CircleShape powerUp;
    powerUp.setRadius(15);
    powerUp.setFillColor(sf::Color::Green);
    powerUp.setPosition(-100, -100); // Start off-screen

    sf::Clock powerUpClock;
    sf::Time powerUpSpawnTime = sf::seconds(10.f);
    ...
    while (window.isOpen())
    {
        ...
        if (isPlaying)
        {
            ...
            // Spawn and manage power-up item
            if (powerUpClock.getElapsedTime() > powerUpSpawnTime && !powerUpActive)
            {
                powerUpClock.restart();
                powerUp.setPosition(std::rand() % static_cast<int>(gameWidth - powerUp.getRadius() * 2), std::rand() % static_cast<int>(gameHeight - powerUp.getRadius() * 2));
                powerUpActive = true;
            }

            // Check collision with power-up
            if (powerUpActive && ball.getGlobalBounds().intersects(powerUp.getGlobalBounds()))
            {
                powerUpActive = false;
                powerUp.setPosition(-100, -100); // Hide power-up

                // Enlarge player's paddle
                leftPaddle.setSize(sf::Vector2f(paddleSize.x, paddleSize.y + 50));
                leftPaddle.setOrigin(paddleSize.x / 2.f, (paddleSize.y + 50) / 2.f);
            }
        ...
        if (isPlaying)
        {
            // Draw the paddles, ball, and power-up
